...
input(Data = #data{secret_key=SK, n=N, f=_F}, Txn) ->
    ...
    case queue:len(Buf) > ?BATCH_SIZE andalso Data#data.acs == undefined of
        true ->
            ...
            _EncBundle = encrypt(SK, term_to_binary(Bundle)),
            ok;
        false ->
...
hbbft_input_test() ->
    ...
    %% bunch of random txns
    Txns = [ crypto:strong_rand_bytes(512) || _ <- lists:seq(1, N)],
    %% some identifiers
    Js = lists:seq(1, length(PrivateKeys)),
    %% zipped initial states for hbbft
    States = [init(SK, N, F, J) || {J, SK} <- lists:zip(Js, PrivateKeys)],
    %% states bundled with txns
    MixedList = lists:zip(States, Txns),
    Res = lists:map(fun({State, Txn}) ->
                            {NewState, Result} = input(State, Txn),
                            {NewState, Result}
                    end, MixedList),
