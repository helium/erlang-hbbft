...
-record(hbbft_serialized_data, {
          ...
          enc_keys = #{} :: #{non_neg_integer() => binary()},
...
handle_msg(Data = #hbbft_data{round=R}, J, {{acs, R}, ACSMsg}) ->
    ...
    case hbbft_acs:handle_msg(Data#hbbft_data.acs, J, ACSMsg) of
        ...
        {NewACS, {result_and_send, Results0, {send, ACSResponse}}} ->
            ...
            VerifiedShares = maps:map(fun({I, _}, {undefined, Share}) when I /= J -> %% don't bother validating our own shares
...
handle_msg(Data = #hbbft_data{round=R}, J, {dec, R, I, Share}) ->
    ...
    case maps:is_key(I, Data#hbbft_data.decrypted) %% have we already decrypted for this instance?
        ...
        false ->
            ...
            NewShares = maps:map(fun({I1, _}, {undefined, AShare}) when I1 /= J -> %% don't bother validating our own shares
                        ...
                        undefined ->
                            ...
                            case length(SharesForThisBundle) > 2 * Data#hbbft_data.f of
                                true ->
                                    ...
                                    check_completion(Data#hbbft_data{dec_shares=NewShares, decrypted=NewDecrypted, failed_combine=[I|Data#hbbft_data.failed_combine]});
                                false ->
                        ...
                        DecKey ->
                            ...
                            case decrypt(DecKey, Enc) of
                                error ->
                                    ...
                                    NewDecrypted = maps:put(I, [], Data#hbbft_data.decrypted),
                                    check_completion(Data#hbbft_data{dec_shares=NewShares, decrypted=NewDecrypted, failed_decrypt=[I|Data#hbbft_data.failed_decrypt]});
                                Decrypted ->
...
-spec deserialize(map(), tpke_privkey:privkey()) -> hbbft_data().
deserialize(M0, SK) ->
