...
simple_test(Config) ->
    ...
    Workers = [
        {Node,
            rpc:call(Node, hbbft_worker, start_link, [
                N,
                F,
                I,
    ...
    lists:foreach(
        fun(Msg) ->
            ...
            [
                hbbft_worker:submit_transaction(Msg, Destination)
             || {_Node, {ok, Destination}} <- Destinations
            ]
        end,
        Msgs
    ),
    ...
    lists:foreach(
        fun(Chain) ->
            ...
            true = hbbft_worker:verify_chain(Chain, hd(KeyShares)),
...
partition_test_(Config, Filter) ->
    ...
    [
        true = ct_rpc:call(FirstNode, erlang, set_cookie, [PartitionedNode, canttouchthis])
     || PartitionedNode <- PartitionedNodes
    ],
    [
        ct_rpc:call(FirstNode, erlang, disconnect_node, [PartitionedNode])
     || PartitionedNode <- PartitionedNodes
    ],
    true = lists:all(fun(E) -> E == pang end, [
        ct_rpc:call(FirstNode, net_adm, ping, [PartitionedNode])
     || PartitionedNode <- PartitionedNodes
    ]),
    true = lists:all(fun(E) -> E == pang end, [
        ct_rpc:call(PartitionedNode, net_adm, ping, [FirstNode])
     || PartitionedNode <- PartitionedNodes
    ]),
    ...
    case Filter of
        ...
        _ ->
            [
                hbbft_worker:set_filter(Filter, Worker)
             || {Node, {ok, Worker}} <- Workers, lists:member(Node, OtherNodes)
            ]
    end,
    ...
    %% wait for all the worker's mailboxes to settle and.
    %% wait for the chains to converge
    Done = hbbft_ct_utils:wait_until(
        fun() ->
            Chains = lists:map(
                fun({_Node, {ok, W}}) ->
                    {ok, Blocks} = hbbft_worker:get_blocks(W),
